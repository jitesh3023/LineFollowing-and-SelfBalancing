...
*/
float opt = 65;
float lower_pwm_constrain =55;
float higher_pwm_constrain = 75;
...
void junction()
{
	...
	int sumvalue=0 , thresholdvalue=0;
	int count = 0 , lastseen =0;

	while(sensor_value[0]<450 && sensor_value[3]<450)
	{
	...
	while(sensor_value[0]>850 && sensor_value[3]>850)
	{
	...
	while(count<4)
	{
		if (sensor_value[0]>thresholdvalue && sensor_value[3]>thresholdvalue /* condition */)
